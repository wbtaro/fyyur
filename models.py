...
venue_genres = db.Table('venue_genres',
  ...
  db.Column('genre', db.String, db.ForeignKey('Genre.name', ondelete="cascade"), primary_key=True)
)
...
class Venue(db.Model):
  ...
  genres =db.relationship('Genre', secondary=venue_genres, backref=db.backref('venues', lazy=True), cascade="all, delete")
...
class Artist(db.Model):
  ...
  seeking_description = db.Column(db.String(200))
  genres =db.relationship('Genre', secondary=artist_genres, backref=db.backref('artists', lazy=True), cascade="all, delete")
