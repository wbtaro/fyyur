...
def search_venues():
  ...
  venues = Venue.query.filter(Venue.name.ilike('%' + search_term + '%')).order_by(Venue.name).all()
  response={
    "count": len(venues),
    "data": []
  }
...
def search_artists():
  search_term = request.form.get('search_term', '')
  artists = Artist.query.filter(Artist.name.ilike('%' + search_term + '%')).order_by(Artist.name).all()
  response = {
    "count": len(artists),
    "data": []
  }
